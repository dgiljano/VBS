void mjj_all_weights_vs_positive_only()
{
    ...
    TFile *f2016_all = new TFile("./all/TMVA.root");
    TFile *f2016_positive = new TFile("./positive_only/TMVA.root");
    ...
    c1->cd();
    hmjj_all->SetTitle("mjj 2016 (signal)");
    hmjj_all->GetYaxis()->SetRangeUser(0,0.35);
    if (normalise)
    {
    ...
    c2->cd();
    hmjj_all->SetTitle("mjj 2016 (background)");
