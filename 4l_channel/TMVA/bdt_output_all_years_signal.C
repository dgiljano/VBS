void bdt_output_all_years_signal()
{
    gStyle->SetOptStat(0);
    ...
    TH1D *hROC_all_2016 = (TH1D*)gDirectory->Get("MVA_BDT_S");
    hROC_all_2016->SetLineColor(kRed);

    f2017_all->cd("dataset/Method_BDT/BDT");
    TH1D *hROC_all_2017 = (TH1D*)gDirectory->Get("MVA_BDT_S");
    hROC_all_2017->SetLineColor(kGreen);

    f2018_all->cd("dataset/Method_BDT/BDT");
    TH1D *hROC_all_2018 = (TH1D*)gDirectory->Get("MVA_BDT_S");
    hROC_all_2018->SetLineColor(kBlue);

    TCanvas *c1 = new TCanvas();
    c1->cd();
    hROC_all_2016->SetTitle("BDT output signal (all weights)");
    hROC_all_2016->Draw();
    ...
    TH1D *hROC_positive_2016 = (TH1D*)gDirectory->Get("MVA_BDT_S");
    hROC_positive_2016->SetLineColor(kRed);

    f2017_positive->cd("dataset/Method_BDT/BDT");
    TH1D *hROC_positive_2017 = (TH1D*)gDirectory->Get("MVA_BDT_S");
    hROC_positive_2017->SetLineColor(kGreen);

    f2018_positive->cd("dataset/Method_BDT/BDT");
    TH1D *hROC_positive_2018 = (TH1D*)gDirectory->Get("MVA_BDT_S");
    hROC_positive_2018->SetLineColor(kBlue);

    TCanvas *c2 = new TCanvas();
    c2->cd();
    hROC_positive_2016->SetTitle("BDT output signal (positive weights only)");
