void mjj_all_years_background()
{
    ...
    TH1D *hmjj_all_2016 = (TH1D*)gDirectory->Get("mjj__Background");
    hmjj_all_2016->SetLineColor(kRed);

    f2017_all->cd("dataset/Method_BDT/BDT");
    TH1D *hmjj_all_2017 = (TH1D*)gDirectory->Get("mjj__Background");
    hmjj_all_2017->SetLineColor(kGreen);

    f2018_all->cd("dataset/Method_BDT/BDT");
    TH1D *hmjj_all_2018 = (TH1D*)gDirectory->Get("mjj__Background");
    hmjj_all_2018->SetLineColor(kBlue);

    TCanvas *c1 = new TCanvas();
    c1->cd();
    hmjj_all_2016->SetTitle("mjj background (all weights)");
    if (normalise)
	{
	...
	hmjj_all_2016->GetYaxis()->SetRangeUser(0,0.6);
    hmjj_all_2016->GetXaxis()->SetRangeUser(0,2000);
    hmjj_all_2016->Draw();
    ...
    TH1D *hmjj_positive_2016 = (TH1D*)gDirectory->Get("mjj__Background");
    hmjj_positive_2016->SetLineColor(kRed);

    f2017_positive->cd("dataset/Method_BDT/BDT");
    TH1D *hmjj_positive_2017 = (TH1D*)gDirectory->Get("mjj__Background");
    hmjj_positive_2017->SetLineColor(kGreen);

    f2018_positive->cd("dataset/Method_BDT/BDT");
    TH1D *hmjj_positive_2018 = (TH1D*)gDirectory->Get("mjj__Background");
    hmjj_positive_2018->SetLineColor(kBlue);

    TCanvas *c2 = new TCanvas();
    c2->cd();
    hmjj_positive_2016->SetTitle("mjj background (positive weights only)");
	if (normalise)
	{
	...
	hmjj_positive_2016->GetYaxis()->SetRangeUser(0,0.6);
    hmjj_positive_2016->GetXaxis()->SetRangeUser(0,2000);
